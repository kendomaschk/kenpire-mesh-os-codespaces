...
class MeshOrchestrator:
    ...
    async def register_node(self, node_id: str, node_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.mesh_stats["active_nodes"] = len([n for n in self.nodes.values() if n["status"] == "active"])
    ...
    async def achieve_consensus(self, proposal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if consensus_achieved:
            ...
            logger.warning(f"Consensus failed: {votes}/{len(active_nodes)} votes (required: {required_votes})")
    ...
    def get_mesh_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "nodes": {node_id: {"status": node["status"], "votes": node["consensus_votes"]} 
